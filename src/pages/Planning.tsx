      ...
      <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6 animate-fade-in">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-3xl bg-accent/15 flex items-center justify-center">
        ...
        <div className="flex items-center gap-1 p-1 bg-white/70 backdrop-blur-sm rounded-2xl shadow-soft w-fit">
          {TABS.map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`px-5 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${activeTab === tab.key ? "gradient-primary text-primary-foreground shadow-soft" : "text-muted-foreground hover:text-foreground"}`}>
              {tab.label}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3 card-soft overflow-hidden">
            <div className="relative">
              {hours.map((hour) => (
                <div key={hour} className="flex border-b border-border/20 last:border-0">
                  ...
                  <div className="flex-1 relative min-h-[64px] border-l border-border/20">
                    {mappedEvents
                      ...
                      .map((event, i) => (
                        <div key={i} className={`absolute left-2 right-2 rounded-2xl px-4 py-2.5 border ${typeStyles.deep}`}
                ...
                {unplannedTasks.map((task) => (
                  <div key={task.id} className="p-3 rounded-2xl border border-dashed border-border hover:border-primary/30 hover:bg-primary/5 transition-all duration-200">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <button onClick={() => handlePlanToday(task.id)} className="pill px-3 py-1 text-[11px] font-bold text-primary bg-primary/10 hover:bg-primary/20 transition-all">Aujourd'hui</button>
                    </div>
                  </div>
                ))}
              </div>
