            ...
            return (
              <div key={task.id} className="bg-card rounded-xl border border-border p-5 card-shadow hover:card-shadow-hover transition-shadow">
                <div className="flex items-start gap-4">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${task.email_id ? "bg-primary/10" : "bg-secondary/10"}`}>
                    {task.email_id ? <Mail className="w-5 h-5 text-primary" /> : <Sparkles className="w-5 h-5 text-secondary" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[11px] font-medium px-2 py-0.5 rounded-full bg-secondary/10 text-secondary">IA</span>
                    ...
                    <div className="flex items-center gap-3 text-xs text-muted-foreground">
                      ...
                      <span>·</span>
                      <span>Durée : {task.estimated_duration ? `${task.estimated_duration} min` : '-'}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <button onClick={() => handleAccept(task.id)} className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-success text-success-foreground text-xs font-medium hover:opacity-90 transition-opacity">
                      <Check className="w-3.5 h-3.5" />Accepter
                    </button>
                    <button onClick={() => handleDone(task.id)} className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-border text-xs font-medium text-foreground hover:bg-accent transition-colors">
                      <Pencil className="w-3.5 h-3.5" />Terminer
                    </button>
                    <button onClick={() => handleArchive(task.id)} className="p-2 rounded-lg border border-border text-muted-foreground hover:text-foreground hover:bg-accent transition-colors">
