...
function WeekView({ events, weekStart, onDayClick }: { events: CalendarEvent[]; weekStart: Date; onDayClick?: (date: Date) => void }) {
  ...
  const eventsByDay = useMemo(() => {
    ...
    for (const e of events) { const key = new Date(e.start_time).toISOString().split("T")[0]; if (!map[key]) map[key] = []; map[key].push(e); }
    return map;
  }, [events]);
    ...
    <div className="card-soft overflow-hidden">
      <div className="grid grid-cols-7 divide-x divide-border/20">
        {days.map(day => {
          ...
          return (
            <div key={key} className={`min-h-[200px] p-2 cursor-pointer hover:bg-primary/5 transition-colors ${isToday(day) ? "bg-primary/5" : ""}`} onClick={() => onDayClick?.(day)}>
  ...
  return (
    <div className="card-soft overflow-hidden">
      <div className="grid grid-cols-7 border-b border-border/20">
        {DAY_NAMES_SHORT.map(d => <div key={d} className="text-center py-2 text-[11px] font-semibold text-muted-foreground">{d}</div>)}
      </div>
...
const StructurePlanning = () => {
  ...
  const { rangeStart, rangeEnd } = useMemo(() => {
    ...
    return { rangeStart: format(startOfWeek(ms, { weekStartsOn: 1 }), "yyyy-MM-dd"), rangeEnd: format(endOfWeek(me, { weekStartsOn: 1 }), "yyyy-MM-dd") };
  }, [activeTab, currentDate, today, selectedDay, viewDateStr]);

  const { data: events = [] } = useCalendarEventsByStructureRange(id || "", rangeStart, rangeEnd);
  ...
  const dateLabel = selectedDay
    ...
    : activeTab === "today" ? format(new Date(), "EEEE d MMMM", { locale: fr }) : activeTab === "week" ? `Semaine du ${format(startOfWeek(currentDate, { weekStartsOn: 1 }), "d MMM", { locale: fr })}` : format(currentDate, "MMMM yyyy", { locale: fr });
                  ...
                  {unplannedTasks.slice(0, 6).map(task => (
                    <HoverCard key={task.id} className="p-3 rounded-2xl border border-dashed border-border hover:border-primary/30 transition-all">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <span className={`pill text-[10px] font-bold px-2 py-0.5 ${task.priority === "high" ? "bg-destructive/20 text-destructive-foreground" : "bg-muted text-muted-foreground"}`}>{task.priority}</span>
                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => handlePlanToday(task.id)} className="pill px-3 py-1 text-[11px] font-bold text-primary bg-primary/10 hover:bg-primary/20 transition-all">Aujourd'hui</motion.button>
                      </div>
                    </HoverCard>
                  ))}
                </div>
