...
const StructureInbox = () => {
  const { id } = useParams<{ id: string }>();
  const { data: allTasks = [], isLoading } = useTasksByStructure(id || "");
  const inboxTasks = allTasks.filter(t => t.is_inbox);
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {task.action_type !== "OTHER" && <span className="pill text-[10px] font-semibold px-2 py-0.5 bg-muted text-muted-foreground">{task.action_type}</span>}
                        </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => handleAccept(task.id)} className="flex items-center gap-1.5 px-3.5 py-2 rounded-2xl bg-success/15 text-success-foreground text-xs font-bold hover:bg-success/25 transition-all">
                          <Check className="w-3.5 h-3.5" />Accepter
                        </motion.button>
                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => handlePlan(task.id)} className="flex items-center gap-1.5 px-3.5 py-2 rounded-2xl bg-accent/15 text-accent text-xs font-bold hover:bg-accent/25 transition-all">
                          <Clock className="w-3.5 h-3.5" />Planifier
                        </motion.button>
                        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => handleIgnore(task.id)} className="p-2 rounded-2xl bg-muted text-muted-foreground hover:text-foreground transition-all">
                          <X className="w-4 h-4" />
                        </motion.button>
                      </div>
                    </div>
