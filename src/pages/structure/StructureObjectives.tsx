...
function GoalCard({ goal, subGoals, tasks, onDecompose, decomposing }: {
  goal: Goal; subGoals: Goal[]; tasks: { action_label: string; status: string }[];
  onDecompose: (id: string) => void; decomposing: string | null;
}) {
  ...
  return (
    <HoverCard className="card-soft p-6">
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        {goal.kpi && <span className="text-[10px] font-medium px-2 py-0.5 rounded-lg bg-accent/50 text-accent-foreground">📊 {goal.kpi} {goal.kpi_unit && `(${goal.kpi_unit})`}</span>}
        {goal.end_date && <span className="text-[10px] font-medium px-2 py-0.5 rounded-lg bg-accent/50 text-accent-foreground">📅 {goal.end_date}</span>}
      </div>
      {goal.success_criteria && <p className="text-[11px] text-muted-foreground mb-3 italic">✅ {goal.success_criteria}</p>}
      <div className="flex items-center gap-4 mb-3">
        ...
        <div className="flex-1">
          ...
          {tasks.length > 0 && <p className="text-[10px] text-muted-foreground mt-1">🎯 {doneTasks}/{tasks.length} tâches complétées</p>}
        </div>
      </div>
      <div className="flex items-center gap-2">
        {subGoals.length === 0 && goal.status !== "completed" && (
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => onDecompose(goal.id)} disabled={decomposing === goal.id}
            ...
            <Sparkles className="w-3.5 h-3.5" />{decomposing === goal.id ? "Décomposition MCII..." : "Décomposer (MCII)"}
          </motion.button>
        )}
        {subGoals.length > 0 && (
            ...
            {subGoals.map(sg => {
              ...
              return (
                <div key={sg.id} className="p-3 rounded-xl bg-muted/30">
                  <div className="flex justify-between items-start mb-1"><h4 className="text-xs font-bold text-foreground">{sg.title}</h4><span className="text-[10px] font-bold text-primary">{sgPct}%</span></div>
                  {sg.description && <p className="text-[10px] text-muted-foreground">{sg.description}</p>}
                  <div className="h-1.5 bg-muted rounded-pill overflow-hidden mt-2"><div className="h-full gradient-primary rounded-pill" style={{ width: `${sgPct}%` }} /></div>
                </div>
              );
            })}
          </motion.div>
...
function CreateGoalDialog({ structureId, onCreated }: { structureId: string; onCreated: () => void }) {
  ...
  const isValid = form.title.trim() && form.kpi.trim() && form.end_date && form.success_criteria.trim();
  ...
  const handleSubmit = () => {
    ...
    createGoal.mutate({
      ...
      difficulty: form.difficulty, period: form.period, structure_id: structureId,
    }, {
      onSuccess: () => { toast.success("Objectif créé selon Locke & Latham 🎯"); setOpen(false); onCreated(); setForm({ title: "", description: "", kpi: "", kpi_unit: "", target_value: "", end_date: "", success_criteria: "", difficulty: "medium", period: "monthly" }); },
    });
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-3 mt-2">
          <div><label className="text-xs font-bold text-foreground">Titre spécifique *</label><Input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="Ex: Obtenir la certification Qualiopi" className="mt-1" /></div>
          <div><label className="text-xs font-bold text-foreground">Description</label><Textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="Contexte et obstacles anticipés (MCII)" className="mt-1" rows={2} /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="text-xs font-bold text-foreground">KPI *</label><Input value={form.kpi} onChange={e => setForm({ ...form, kpi: e.target.value })} placeholder="Ex: Score audit" className="mt-1" /></div>
            <div><label className="text-xs font-bold text-foreground">Unité</label><Input value={form.kpi_unit} onChange={e => setForm({ ...form, kpi_unit: e.target.value })} placeholder="Ex: %, points" className="mt-1" /></div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="text-xs font-bold text-foreground">Valeur cible</label><Input type="number" value={form.target_value} onChange={e => setForm({ ...form, target_value: e.target.value })} placeholder="100" className="mt-1" /></div>
            <div><label className="text-xs font-bold text-foreground">Échéance *</label><Input type="date" value={form.end_date} onChange={e => setForm({ ...form, end_date: e.target.value })} className="mt-1" /></div>
          </div>
          <div><label className="text-xs font-bold text-foreground">Critères de réussite *</label><Textarea value={form.success_criteria} onChange={e => setForm({ ...form, success_criteria: e.target.value })} placeholder="Comment saurez-vous que l'objectif est atteint ?" className="mt-1" rows={2} /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="text-xs font-bold text-foreground">Difficulté</label><Select value={form.difficulty} onValueChange={v => setForm({ ...form, difficulty: v })}><SelectTrigger className="mt-1"><SelectValue /></SelectTrigger><SelectContent>{DIFFICULTIES.map(d => <SelectItem key={d.key} value={d.key}>{d.label}</SelectItem>)}</SelectContent></Select></div>
            <div><label className="text-xs font-bold text-foreground">Période</label><Select value={form.period} onValueChange={v => setForm({ ...form, period: v })}><SelectTrigger className="mt-1"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="weekly">Semaine</SelectItem><SelectItem value="monthly">Mois</SelectItem><SelectItem value="quarterly">Trimestre</SelectItem></SelectContent></Select></div>
          </div>
          ...
          {!isValid && <p className="text-[10px] text-destructive text-center">* Titre, KPI, échéance et critères obligatoires (Locke & Latham)</p>}
        </div>
      </DialogContent>
    </Dialog>
  );
...
const StructureObjectives = () => {
  ...
  const { data: tasks = [] } = useTasks({ structureId: id });
  ...
  const handleDecompose = async (goalId: string) => {
    ...
    try {
      ...
      toast.success(`MCII : ${data.sub_goals_created} sous-objectifs + ${data.tasks_created} tâches 🧩`);
    ...
    <PageTransition>
      <div className="p-6 lg:p-8 max-w-5xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          ...
          {id && <CreateGoalDialog structureId={id} onCreated={() => qc.invalidateQueries({ queryKey: ["goals"] })} />}
        </div>
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {filtered.map(g => (
            <StaggerItem key={g.id}>
              <GoalCard goal={g} subGoals={goals.filter(sg => sg.parent_goal_id === g.id)} tasks={tasks.filter(t => t.structure_id === g.structure_id)} onDecompose={handleDecompose} decomposing={decomposing} />
            </StaggerItem>
          ))}
        </StaggerContainer>
      </div>
...
export default StructureObjectives;
