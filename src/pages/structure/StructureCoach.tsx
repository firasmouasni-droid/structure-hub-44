...
import { useTasksByStructure } from "@/hooks/useTasks";
...
const StructureCoach = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: "assistant", content: `Bonjour ! 👋 Je suis ton coach IA pour **${structure?.name || 'cet espace'}**. Je connais tes tâches, objectifs et planning. Que puis-je faire pour toi ?` },
  ]);
  ...
  useEffect(() => {
    if (structure) {
      setMessages([{ role: "assistant", content: `Bonjour ! 👋 Je suis ton coach IA pour **${structure.name}**. Je connais tes tâches, objectifs et planning. Que puis-je faire pour toi ?` }]);
    }
  }, [structure?.name]);
  ...
  const buildContext = () => {
    ...
    return {
      ...
      today,
      tasks: { total: tasks.length, todayTotal: todayTasks.length, todayDone: doneTasks.length, highPriority: highPriority.length, unplanned: tasks.filter(t => !t.due_date && t.status !== "done").length },
  ...
  const quickActions = [
    ...
    { icon: <Sparkles className="w-4 h-4" />, label: "Montre mes objectifs" },
  ];
    ...
    <div className="flex flex-col h-full">
        <motion.div className="border-b border-border/30 bg-card/80 backdrop-blur-sm px-6 py-4" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-3 max-w-4xl mx-auto">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Contexte : {structure?.name || '...'}</p>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div className="border-t border-border/30 bg-card/80 backdrop-blur-sm p-4" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <div className="max-w-4xl mx-auto flex gap-3">
            <input type="text" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && sendMessage()} placeholder="Demande à ton coach IA..." className="flex-1 px-5 py-3.5 rounded-2xl border border-border bg-card/90 text-sm text-foreground placeholder:text-muted-foreground shadow-inner-soft focus:outline-none focus:ring-2 focus:ring-primary/30" />
