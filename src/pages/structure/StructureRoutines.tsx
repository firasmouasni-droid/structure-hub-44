...
import { PageTransition, StaggerContainer, StaggerItem, FadeInSection } from "@/components/motion/MotionWrappers";
import { motion } from "framer-motion";

const StructureRoutines = () => {
  const { id } = useParams<{ id: string }>();
  const { data: routines = [] } = useRoutines();
  const createRoutine = useCreateRoutine();
  const updateRoutine = useUpdateRoutine();

  const routine = routines.find(r => r.structure_id === id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("Routines sauvegardées !");
    } catch {
        ...
        <div className="p-6 lg:p-8 max-w-4xl mx-auto space-y-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Règles automatiques pour organiser ta journée</p>
            </div>
          </div>
            ...
            <StaggerItem>
              <div className="card-soft p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Début</label>
                    <input type="time" value={morningStart} onChange={e => setMorningStart(e.target.value)} className="w-full px-4 py-3 rounded-2xl border border-border bg-card/90 text-sm" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Fin</label>
                    <input type="time" value={morningEnd} onChange={e => setMorningEnd(e.target.value)} className="w-full px-4 py-3 rounded-2xl border border-border bg-card/90 text-sm" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Type</label>
                    <select value={morningFocus} onChange={e => setMorningFocus(e.target.value)} className="w-full px-4 py-3 rounded-2xl border border-border bg-card/90 text-sm">
                      <option value="deep_work">Deep Work</option>
                      <option value="urgent_tasks">Tâches urgentes</option>
                      <option value="creative">Travail créatif</option>
                    </select>
                  </div>
                </div>
              </div>
            ...
            <StaggerItem>
              <div className="card-soft p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Début</label>
                    <input type="time" value={afternoonStart} onChange={e => setAfternoonStart(e.target.value)} className="w-full px-4 py-3 rounded-2xl border border-border bg-card/90 text-sm" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Fin</label>
                    <input type="time" value={afternoonEnd} onChange={e => setAfternoonEnd(e.target.value)} className="w-full px-4 py-3 rounded-2xl border border-border bg-card/90 text-sm" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Type</label>
                    <select value={afternoonFocus} onChange={e => setAfternoonFocus(e.target.value)} className="w-full px-4 py-3 rounded-2xl border border-border bg-card/90 text-sm">
                      <option value="meetings_admin">Meetings & Admin</option>
                      <option value="deep_work">Deep Work</option>
                      <option value="collaborative">Travail collaboratif</option>
                    </select>
                  </div>
                </div>
              </div>
...
export default StructureRoutines;
