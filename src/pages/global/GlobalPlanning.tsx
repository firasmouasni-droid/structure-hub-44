...
import { CalendarDays, Sparkles, Loader2, Brain, Coffee, Mail } from "lucide-react";
...
// Parse routine zones from routine config
function getRoutineZones(routine: any) {
  ...
  if (af?.start && af?.end) {
    zones.push({
      ...
      label: af.focus === "meetings_admin" ? "Meetings & Admin" : af.focus === "collaborative" ? "Collaboratif" : "Admin",
      type: "admin",
      icon: "☕",
    });
  }
...
const GlobalPlanning = () => {
  ...
  // Get the routine zone for a given hour (if any)
